...
docker pull gcr.io/phading-prod/wait-list-ui:latest

# Run docker container from image in docker hub
docker run   --name=$CONTAINER_NAME   --privileged   --restart=always   --tty   --detach   --network="host"   --log-driver=gcplogs   gcr.io/phading-prod/wait-list-ui:latest
